...
function [dip, Z] = visualizeCloud2D(X, n, diameter, angle, caption)
    ...
    dip = dipshow(Z','lin');
%     dipmapping(dip,[0 0.7*max(Z(:))],'COLORMAP',hot(256))
    dipmapping(dip,[0 1.1*max(Z(:))],'COLORMAP',hot(256))
    
%     bar = 20*n/diameter/130; %20nm bar
%    bar = 100*n/diameter/105; %20nm bar
...
%     text(n-92,n-80, '0.25 pix','Color','white','FontSize',15,'FontWeight' ...
%          ,'bold','HorizontalAlignment','center','VerticalAlignment' ...
%          ,'bottom');
%     rectangle('Position',[n-140,n-70,100,20],'FaceColor','white')    
