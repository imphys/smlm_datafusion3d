...
function [ superParticle, MT, outParticles] = one2all_modified(Particles, iter, oldM, outdir)
    ...
    superParticle{1}.points = initParticle.points;
    ...
    for j=1:iter
        ...
        for i=1:N
            ...
            [parameter{j,i}, ~, ~, ~, fval] = pairFitting_parallel(M, S, scale(iter));
            outParticles{j,i}.poinst = transform_by_rigid2d(M.points, parameter{j,i});
            outParticles{j,i}.sigma = M.sigma;
            ...
            tmpParticle.val = [tmpParticle.val;fval];

        end

        a = toc;
        disp(['iter #' num2str(j) '... done in ' num2str(a) ' seconds']); 
        superParticle{j+1}.points = tmpParticle.points; 
        superParticle{j+1}.val = tmpParticle.val; 
