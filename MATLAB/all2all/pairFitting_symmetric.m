...
function [parameter, registered_model, history, config, max_value] = pairFitting_symmetric(M, S, scale) 
    ...
    %scale = [0.5];                 % the set of scales in a multiscale 
...
%         param{iter} = [0, 0, init_angle(iter)];
%         tmpParticle = transform_by_rigid2d(M_resampled.points, param{iter});
%         [f(iter), ~] =  GaussTransform(tmpParticle, S_resampled.points, f_config.scale);
...
%     newAngle = angle;
%     newAngle(IDX) = [];
%     finalAngleID = randperm(size(f,2)-1,1);

    newAngle = angle;
%     newAngle(IDX) = [];
%     finalAngleID = randperm(size(f,2),1);
    finalAngleID = randperm(size(fval,2),1);
    
    parameter = param{1,IDX};
    parameter = [0, 0, newAngle(finalAngleID)+parameter(1,3)];
